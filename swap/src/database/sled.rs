...
impl Database for SledDatabase {
    ...
    async fn get_peer_id(&self, swap_id: Uuid) -> Result<PeerId> {
        ...
        let encoded = self
            .peers
            .get(&key)?
            .ok_or_else(|| anyhow!("No peer-id found for swap id {} in sled", swap_id))?;
    ...
    async fn get_monero_address(&self, swap_id: Uuid) -> Result<monero::Address> {
        let encoded = self
            ...
            .ok_or_else(|| {
                anyhow!(
                    "No Monero address found for swap id {} in sled",
                    swap_id
                )
            })?;
    ...
    async fn get_state(&self, swap_id: Uuid) -> Result<State> {
        ...
        let encoded = self
            .swaps
            .get(&key)?
            .ok_or_else(|| anyhow!("Swap with id {} not found in sled", swap_id))?;
...
mod tests {
    ...
    async fn can_write_twice_to_one_key() {
        ...
        // We insert and recover twice to ensure sled implementation allows the
