...
use crate::harness::{setup_test, SlowCancelConfig};
...
pub async fn initialize_daemon(context: Context) -> (Client, MakeCapturingWriter, Arc<Context>) {
...
pub async fn get_bitcoin_balance() {
    setup_test(SlowCancelConfig, |harness_ctx| async move {
        let context = harness_ctx.get_bob_context().await;
        let (client, _, _) = initialize_daemon(context).await;
...
pub async fn get_bitcoin_balance_with_log_reference_id() {
    setup_test(SlowCancelConfig, |harness_ctx| async move {
        let context = harness_ctx.get_bob_context().await;
        let (client, writer, _) = initialize_daemon(context).await;
...
pub async fn get_history() {
    setup_test(SlowCancelConfig, |mut harness_ctx| async move {
        ...
        assert_eq!(
            response,
            HashMap::from([("swaps".to_string(), swaps)])
        );
...
pub async fn get_raw_states() {
    setup_test(SlowCancelConfig, |mut harness_ctx| async move {
        ...
        let response: HashMap<String, HashMap<Uuid, Vec<Value>>> =
            client.request("get_raw_states", ObjectParams::new()).await.unwrap();
...
pub async fn get_swap_info_valid_swap_id() {
    setup_test(SlowCancelConfig, |mut harness_ctx| async move {
        ...
        assert_has_keys_hashmap(
            &response,
            &["txRefundFee", "swapId", "cancelTimelock", "timelock", "punishTimelock",
                "stateName", "btcAmount", "startDate", "btcRefundAddress", "txCancelFee",
                "xmrAmount", "completed", "txLockId", "seller"]
        );

        // Assert specific fields
        assert_eq!(response.get("swapId").unwrap(), &bob_swap_id.to_string());
        assert_eq!(response.get("stateName").unwrap(), &"btc is locked".to_string());
        assert_eq!(response.get("completed").unwrap(), &Value::Bool(false));

        // Check seller object and its keys
        let seller = response.get("seller").expect("Field 'seller' is missing from response").as_object().expect("'seller' is not an object");
        assert_has_keys_serde(seller, &["peerId"]);

        // Check timelock object, nested 'None' object, and blocks_left
        let timelock = response.get("timelock").expect("Field 'timelock' is missing from response").as_object().expect("'timelock' is not an object");
        let none_obj = timelock.get("None").expect("Field 'None' is missing from 'timelock'").as_object().expect("'None' is not an object in 'timelock'");
        let blocks_left = none_obj.get("blocks_left").expect("Field 'blocks_left' is missing from 'None'").as_i64().expect("'blocks_left' is not an integer");

        // Validate blocks_left
        assert!(blocks_left > 0 && blocks_left <= 180, "Field 'blocks_left' should be > 0 and <= 180 but got {}", blocks_left);
...
pub async fn list_sellers_missing_rendezvous_point() {
    setup_test(SlowCancelConfig, |harness_ctx| async move {
        ...
        let params = ObjectParams::new();
        let result: Result<HashMap<String, String>, _> = client.request("list_sellers", params).await;
