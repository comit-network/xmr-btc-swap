...
// If Alice stops before learning the encsig that Bob sent, she has to refund
// because the connection with Bob is not re-dialed at the moment!
// TODO: Alice goes till "Alice acknowledged message 3 received" which is a bit
//  awkward. Validate that that this is printed, but the state not saved.
#[tokio::test]
async fn given_alice_restarts_after_xmr_is_locked_refund_swap() {
    ...
    if let swap::state::Swap::Alice(state) = state_before_restart.clone() {
        assert!(matches!(state, swap::state::Alice::XmrLocked {..}));
    }
